...
export default function SearchInput({ isScrolled }: SearchInputProps) {
  ...
  return (
    <div
      className={`${isScrolled ? "h-8" : "h-10"} relative flex w-full items-center gap-x-2 rounded-xl bg-neutral3 px-3 py-2.5 transition-all`}
    >
      <MdOutlineSearch className="text-neutral9 size-5" />
      <input
        type="text"
        placeholder="جستجوی گیاه"
        className="placeholder:text-neutral9 text-neutral11 w-full border-0 outline-0"
      ...
      {isSearchOpen && query && (
        <div className="border-neutral3 custom-scroll ltr absolute top-full right-0 left-0 z-40 mt-1 max-h-80 overflow-y-auto rounded-xl border bg-white p-3 pr-1.5 shadow-lg">
          {isLoading ? (
            <div className="flex items-center justify-center py-4">
              <div className="border-primary size-6 animate-spin rounded-full border-4 border-t-transparent" />
            </div>
          ) : results.length > 0 ? (
