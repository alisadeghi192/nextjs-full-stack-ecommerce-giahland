      ...
      <div className="flex-1 space-y-2">
        {tickets.length === 0 ? (
          ...
          tickets.map((ticket: DashboardTickets) => (
            <div
              key={ticket._id}
              className={`border-neutral3 rounded-2xl border p-3 max-xs:px-2 max-xs:py-1 shadow-lg transition max-sm:p-3 ${ticket.status === "pending" ? "border-yellow-400 bg-yellow-50" : ""}`}
            >
              <div className="flex justify-between">
                <div className="flex items-center flex-1 gap-x-2">
                  <span className="max-xs:text-sm truncate max-w-1/2">{ticket.userName}</span>
                  <div className="flex flex-wrap items-center gap-3">
                    {ticket.status === "pending" ? (
                      <span className="rounded-full bg-yellow-500 px-2 py-0.5 text-xs text-white text-nowrap">
                        بی پاسخ
                      </span>
                    ) : (
                      <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-700 text-nowrap" >
                        پاسخ داده شده
                      </span>
                    )}
                  </div>
                ...
                <div className="text-sm text-neutral9 max-xs:text-xs">
